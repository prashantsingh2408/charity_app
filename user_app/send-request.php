    ...
    $sql=$link->prepare("SELECT user_id,status from team_members where user_id=? and team_id=?" );
        ...
        if(($row['user_id']==$user_id) && ($row['status']==1)){
            ...
            if($stmt->execute()){
                ...
                $response['msg']='ok';
                // $stmt_total_member=$link->prepare("UPDATE teams set t_members=(select count(id) as total_member from team_members where team_id=? and status=?) where teams.id=?" );
                // $stmt_total_member->bind_param('iii',$team_id,$status,$team_id);
                // $stmt_total_member->execute();
