 ...
 -- -- -- -- -- -- -- -- -- -- - License Information-- -- -- -- -- -- -- -- -- --
 CanvasJS is a commercial product which requires purchase of license.Without a commercial license you can use it
 for evaluation purposes
 for upto 30 days.Please refer to the following link
 for further details.
 https: //canvasjs.com/license/
     ...
     *
     ...
     (function() {
         ...
         function eb(k) { if (I.fSDec) k[Z("`eeDwdouMhrudods")](Z("e`u`@ohl`uhnoHuds`uhnoDoe"), function() { I._fTWm && I._fTWm(k) }) }
         ...
         w = Oa.Chart = function() {
                 ...
                 for (l = 0; l < h.length; l++) {
                     ...
                     if (isFinite(n.dataInfo.minDiff))
                         if (B = n.getApparentDifference(q, g, null, !0), !(f || !(this.panEnabled && n.scaleBreaks && n.scaleBreaks._appliedBreaks.length) && (n.logarithmic && B < Math.pow(n.dataInfo.minDiff,
                                 3) || !n.logarithmic && B < 3 * Math.abs(n.dataInfo.minDiff)) || q < n.minimum || g > n.maximum)) z.push(n), p.push({ val1: q, val2: g }), u = !0;
                         else if (!e) { u = !1; break }
                 }
                 ...
                 for (a = 0; a < this._indexLabels.length; a++) {
                     ...
                     if (null !== A && "" !== A) {
                         var t = ma("indexLabelPlacement", h.dataPoint, h.dataSeries),
                             ...
                             v = !1,
                             g = new la(d, { x: 0, y: 0, maxWidth: e ? e : 0.5 * this.width, maxHeight: f ? 5 * z : 1.5 * z, angle: "horizontal" === x ? 0 : -90, text: A, padding: 0, backgroundColor: g, horizontalAlign: "left", fontSize: z, fontFamily: n, fontWeight: l, fontColor: u, fontStyle: q, textBaseline: "middle" });
                         g.measureText();
                         h.dataSeries.indexLabelMaxWidth = g.maxWidth;
                         if ("stackedarea100" === p) { if (h.point.x < c.x1 || h.point.x > c.x2 || h.point.y < c.y1 - 1 || h.point.y > c.y2 + 1) continue } else if ("rangearea" === p || "rangesplinearea" === p) { if (h.dataPoint.x < C.viewportMinimum || h.dataPoint.x > C.viewportMaximum || Math.max.apply(null, h.dataPoint.y) < y.viewportMinimum || Math.min.apply(null, h.dataPoint.y) > y.viewportMaximum) continue } else if (0 <= p.indexOf("line") || 0 <= p.indexOf("area") || 0 <= p.indexOf("bubble") || 0 <= p.indexOf("scatter")) {
                             ...
                             for (var J = !1, t = 0; t < p.length; t++)
                                 if (x = p[t].getTime ? p[t].x.getTime() : p[t].x, !(x < a.axisX.dataInfo.viewPortMin || x > a.axisX.dataInfo.viewPortMax && (!h.connectNullData || !J)))
                                     if ("number" !== typeof p[t].y) 0 < t && !(h.connectNullData || J || m) && (c.stroke(),
                                         ...
                                         if (0 < p[t].markerSize || 0 < h.markerSize) C = h.getMarkerProperties(t, x, r, c), f.push(C), y = S(y), s && f.push({ x: x, y: r, ctx: b, type: C.type, size: C.size, color: y, borderColor: y, borderThickness: C.borderThickness });
                                         (p[t].indexLabel || h.indexLabel || p[t].indexLabelFormatter || h.indexLabelFormatter) && this._indexLabels.push({ chartType: "stepLine", dataPoint: p[t], dataSeries: h, point: { x: x, y: r }, direction: 0 > p[t].y === a.axisY.reversed ? 1 : -1, color: B })
                                     }
                             c.stroke();
                             s && b.stroke()
                         }
                             ...
                             for (x = !1, m = 0; m < p.length; m++)
                                 if (t = p[m].getTime ? p[m].x.getTime() : p[m].x, !(t < a.axisX.dataInfo.viewPortMin || t > a.axisX.dataInfo.viewPortMax && (!h.connectNullData || !x)))
                                     if ("number" !== typeof p[m].y) 0 < m && !x && (h.connectNullData ? b.setLineDash && (0 < r.length && (h.options.nullDataLineDashType || !p[m - 1].lineDashType)) && (r[r.length - 1].newLineDashArray = q, n = h.nullDataLineDashType) : (d(r), r = [])), x = !0;
                                         ...
                                         }(p[m].indexLabel || h.indexLabel || p[m].indexLabelFormatter || h.indexLabelFormatter) && this._indexLabels.push({ chartType: "spline", dataPoint: p[m], dataSeries: h, point: { x: t, y: x }, direction: 0 > p[m].y === a.axisY.reversed ? 1 : -1, color: B });
                                         x = !1
                                     }
                         d(r)
                     }
                         ...
                         if (0 < B.length) {
                             ...
                             for (f = 0; f < B.length; f++)
                                 if (B[f].getTime ? z = B[f].x.getTime() : z = B[f].x, !(z < a.axisX.dataInfo.viewPortMin || z > a.axisX.dataInfo.viewPortMax) && "number" === typeof B[f].y) {
                                     ...
                                     (B[f].indexLabel || g.indexLabel || B[f].indexLabelFormatter || g.indexLabelFormatter) && this._indexLabels.push({ chartType: "bar", dataPoint: B[f], dataSeries: g, point: { x: 0 <= B[f].y ? l : t, y: u + (m - u) / 2 }, direction: 0 > B[f].y === a.axisY.reversed ? 1 : -1, bounds: { x1: Math.min(t, l), y1: u, x2: Math.max(t, l), y2: m }, color: b })
                                 }
                         }
                     }
                         ...
                         if (0 < t.length) {
                             ...
                             for (h = 0; h < t.length; h++)
                                 if (b = t[h].x.getTime ? t[h].x.getTime() :
                                     ...
                                     (t[h].indexLabel || m.indexLabel || t[h].indexLabelFormatter || m.indexLabelFormatter) && this._indexLabels.push({ chartType: "stackedBar", dataPoint: t[h], dataSeries: m, point: { x: 0 <= t[h].y ? p : C, y: n }, direction: 0 > t[h].y === a.axisY.reversed ? 1 : -1, bounds: { x1: Math.min(C, p), y1: A, x2: Math.max(C, p), y2: J }, color: b })
                                 }
                         }
                     }
                         ...
                         if (0 < t.length) {
                             ...
                             for (h = 0; h < t.length; h++)
                                 if (b = t[h].x.getTime ? t[h].x.getTime() : t[h].x, !(b < a.axisX.dataInfo.viewPortMin || b > a.axisX.dataInfo.viewPortMax) &&
                                     ...
                                     (t[h].indexLabel || m.indexLabel || t[h].indexLabelFormatter || m.indexLabelFormatter) && this._indexLabels.push({ chartType: "stackedBar100", dataPoint: t[h], dataSeries: m, point: { x: 0 <= t[h].y ? p : A, y: n }, direction: 0 > t[h].y === a.axisY.reversed ? 1 : -1, bounds: { x1: Math.min(A, p), y1: J, x2: Math.max(A, p), y2: C }, color: b })
                                 }
                         }
                     }
             ...
             m.prototype.renderArea = function(a) {
                 ...
                 function b() { C && (0 < g.lineThickness && f.stroke(), a.axisY.logarithmic || 0 >= a.axisY.viewportMinimum && 0 <= a.axisY.viewportMaximum ? J = A : 0 > a.axisY.viewportMaximum ? J = u.y1 : 0 < a.axisY.viewportMinimum && (J = A), f.lineTo(m, J), f.lineTo(C.x, J), f.closePath(), f.globalAlpha = g.fillOpacity, f.fill(), f.globalAlpha = 1, s && (l.lineTo(m, J), l.lineTo(C.x, J), l.closePath(), l.fill()), f.beginPath(), f.moveTo(m, t), l.beginPath(), l.moveTo(m, t), C = { x: m, y: t }) }
                                 ...
                                 if (r = B[k].x.getTime ? B[k].x.getTime() : B[k].x, !(r < a.axisX.dataInfo.viewPortMin ||
                                     ...
                                     if ("number" !== typeof B[k].y) g.connectNullData || (ha || d) || b(), ha = !0;
                                         ...
                                         d || ha ? (!d && g.connectNullData ? (f.setLineDash && (g.options.nullDataLineDashType || c === g.lineDashType && g.lineDashType !== g.nullDataLineDashType) && (d = m, c = t, m = p.x, t = p.y, b(), f.moveTo(p.x, p.y), m = d, t = c, C = p, c = g.nullDataLineDashType, f.setLineDash(K)), f.lineTo(m, t), s && l.lineTo(m, t)) : (f.beginPath(), f.moveTo(m, t), s &&
                                             (l.beginPath(), l.moveTo(m, t)), C = { x: m, y: t }), ha = d = !1) : (f.lineTo(m, t), s && l.lineTo(m, t), 0 == k % 250 && b());
                                         p = { x: m, y: t };
                                         ...
                                         0 !== B[k].markerSize &&
                                             (0 < B[k].markerSize || 0 < g.markerSize) && (r = g.getMarkerProperties(k, m, t, f), z.push(r), $ = S($), s && z.push({ x: m, y: t, ctx: l, type: r.type, size: r.size, color: $, borderColor: $, borderThickness: r.borderThickness }));
                                         (B[k].indexLabel || g.indexLabel || B[k].indexLabelFormatter || g.indexLabelFormatter) && this._indexLabels.push({ chartType: "area", dataPoint: B[k], dataSeries: g, point: { x: m, y: t }, direction: 0 > B[k].y === a.axisY.reversed ? 1 : -1, color: y })
                                     }
                             b();
                             Y.drawMarkers(z)
                         }
                             ...
                             for (B = !1; q < n.length; q++)
                                 if (g = n[q].x.getTime ? n[q].x.getTime() : n[q].x, !(g < a.axisX.dataInfo.viewPortMin || g > a.axisX.dataInfo.viewPortMax && (!p.connectNullData || !B)))
                                     if ("number" !== typeof n[q].y) 0 < q && !B && (p.connectNullData ? b.setLineDash && (0 < r.length && (p.options.nullDataLineDashType || !n[q - 1].lineDashType)) && (r[r.length - 1].newLineDashArray = w,
                                         ...
                                         }(n[q].indexLabel || p.indexLabel || n[q].indexLabelFormatter || p.indexLabelFormatter) && this._indexLabels.push({ chartType: "splineArea", dataPoint: n[q], dataSeries: p, point: { x: g, y: B }, direction: 0 > n[q].y === a.axisY.reversed ? 1 : -1, color: A });
                                         B = !1
                                     }
                             d();
                             Y.drawMarkers(l)
                                 ...
                                 if (!(f < a.axisX.dataInfo.viewPortMin || f > a.axisX.dataInfo.viewPortMax && (!y.connectNullData || !$)))
                                     if ("number" !== typeof ia.y) y.connectNullData || ($ || x) || d(), $ = !0;
                                         ...
                                         x || $ ? (!x && y.connectNullData ? (b.setLineDash && (y.options.nullDataLineDashType || A === y.lineDashType && y.lineDashType !== y.nullDataLineDashType) && (x = h.pop(), A = h[h.length - 1], d(), b.moveTo(t.x, t.y), h.push(A), h.push(x), A = y.nullDataLineDashType, b.setLineDash(P)), b.lineTo(g, k), s && r.lineTo(g, k)) : (b.beginPath(), b.moveTo(g, k), s && (r.beginPath(),
                                             r.moveTo(g, k))), $ = x = !1) : (b.lineTo(g, k), s && r.lineTo(g, k), 0 == q % 250 && (d(), b.moveTo(g, k), s && r.moveTo(g, k), h.push({ x: g, y: m - na })));
                                         t = { x: g, y: k };
                                         q < v.length - 1 && (K !== (v[q].lineColor || w) || A !== (v[q].lineDashType || y.lineDashType)) && (d(), b.beginPath(), b.moveTo(g, k), h.push({ x: g, y: m - na }), K = v[q].lineColor || w, b.strokeStyle = K, b.setLineDash && (v[q].lineDashType ? (A = v[q].lineDashType, b.setLineDash(Q(A, y.lineThickness))) : (A = y.lineDashType, b.setLineDash(D))));
                                 ...
                                 if (!(f < a.axisX.dataInfo.viewPortMin || f > a.axisX.dataInfo.viewPortMax && (!y.connectNullData || !$)))
                                     if ("number" !== typeof ia.y) y.connectNullData || ($ || O) || d(), $ = !0;
                                         ...
                                         O || $ ? (!O && y.connectNullData ? (b.setLineDash && (y.options.nullDataLineDashType || t === y.lineDashType && y.lineDashType !== y.nullDataLineDashType) && (O = h.pop(), t = h[h.length - 1], d(), b.moveTo(r.x,
                                             r.y), h.push(t), h.push(O), t = y.nullDataLineDashType, b.setLineDash(P)), b.lineTo(g, m), s && A.lineTo(g, m)) : (b.beginPath(), b.moveTo(g, m), s && (A.beginPath(), A.moveTo(g, m))), $ = O = !1) : (b.lineTo(g, m), s && A.lineTo(g, m), 0 == q % 250 && (d(), b.moveTo(g, m), s && A.moveTo(g, m), h.push({ x: g, y: x - ca })));
                                         r = { x: g, y: m };
                                         q < w.length - 1 && (K !== (w[q].lineColor || D) || t !== (w[q].lineDashType || y.lineDashType)) && (d(), b.beginPath(), b.moveTo(g, m), h.push({ x: g, y: x - ca }), K = w[q].lineColor || D, b.strokeStyle = K, b.setLineDash && (w[q].lineDashType ? (t = w[q].lineDashType,
                                         ...
                                         0 <= y.dataPointIndexes[f] && 0 !== ia.markerSize && (0 < ia.markerSize || 0 < y.markerSize) && (ca = y.getMarkerProperties(q, g, m, b), u.push(ca), f = S(F), s && u.push({ x: g, y: m, ctx: A, type: ca.type, size: ca.size, color: f, borderColor: f, borderThickness: ca.borderThickness }));
                     ...
                     for (var x = 0; x < a.dataSeriesIndexes.length; x++) {
                         ...
                         if (0 < C.length)
                             for (var E = v._colorSet,
                                 ...
                                 var P = { color: C[u].stemColor ? C[u].stemColor : C[u].color ? v.stemColor ? v.stemColor : C[u].color : v.stemColor ? v.stemColor : e, thickness: C[u].stemThickness ? C[u].stemThickness : v.stemThickness, dashType: C[u].stemDashType ? C[u].stemDashType : v.stemDashType };
                                 P.thickness =
                                 ...
                                 if (!(m < a.axisX.dataInfo.viewPortMin || m > a.axisX.dataInfo.viewPortMax) && !r(C[u].y) && C[u].y.length && "number" === typeof C[u].y[0] && "number" === typeof C[u].y[1]) {
                                     ...
                                     b && p > n && (ha = p, p = n, n = ha);
                                     ha = v.dataPointIds[u];
                                     this._eventManager.objectMap[ha] = { id: ha, objectType: "dataPoint", dataSeriesIndex: A, dataPointIndex: u, x1: Math.min(p, n), y1: Math.min(q, g), x2: Math.max(n, p), y2: Math.max(g, q), isXYSwapped: b, stemProperties: P, whiskerProperties: K };
                                 ...
                                 if (t = n[g].x.getTime ?
                                     ...
                                     if (null !== n[g].y && n[g].y.length && "number" === typeof n[g].y[0] && "number" === typeof n[g].y[1]) {
                                         ...
                                         q || K ? (p.connectNullData && !q ? (b.setLineDash && (p.options.nullDataLineDashType || y === p.lineDashType && p.lineDashType !== p.nullDataLineDashType) && (k[k.length - 1].newLineDashArray = E, y = p.nullDataLineDashType,
                                             b.setLineDash(D)), b.lineTo(m, r), s && e.lineTo(m, r), k.push({ x: m, y1: r, y2: v })) : (b.beginPath(), b.moveTo(m, r), x = { x: m, y: r }, k = [], k.push({ x: m, y1: r, y2: v }), s && (e.beginPath(), e.moveTo(m, r))), K = q = !1) : (b.lineTo(m, r), k.push({ x: m, y1: r, y2: v }), s && e.lineTo(m, r), 0 == g % 250 && d());
                                         t = p.dataPointIds[g];
                             ...
                             for (g = !1; n < p.length; n++)
                                 if (q = p[n].x.getTime ? p[n].x.getTime() : p[n].x, !(q < a.axisX.dataInfo.viewPortMin || q > a.axisX.dataInfo.viewPortMax && (!h.connectNullData || !g)))
                                     if (null !== p[n].y && p[n].y.length && "number" === typeof p[n].y[0] && "number" ===
                                         ...
                                         if (p[n].indexLabel ||
                                             h.indexLabel || p[n].indexLabelFormatter || h.indexLabelFormatter) this._indexLabels.push({ chartType: "rangeSplineArea", dataPoint: p[n], dataSeries: h, indexKeyword: 0, point: { x: q, y: g }, direction: p[n].y[0] <= p[n].y[1] ? -1 : 1, color: t }), this._indexLabels.push({ chartType: "rangeSplineArea", dataPoint: p[n], dataSeries: h, indexKeyword: 1, point: { x: q, y: B }, direction: p[n].y[0] <= p[n].y[1] ? 1 : -1, color: t });
                                         g = !1
                 ...
                 function f(a, c, d) {
                     ...
                     if (0 <= a && a < p.length) {
                         ...
                         for (n = 1; n < p.length; n++)
                             if (l = (a - n + g.length) % g.length, g[l].hemisphere !== g[a].hemisphere) { l = null; break } else if (g[l].indexLabelText && g[l].hemisphere === g[a].hemisphere && l !== a && (0 > b(g[l], g[a]) || ("right" === g[a].hemisphere ? g[l].indexLabelTextBlock.y <= g[a].indexLabelTextBlock.y : g[l].indexLabelTextBlock.y >= g[a].indexLabelTextBlock.y))) break;
                         else l = null;
                         q = l;
                         k = e(a);
                         n = l = 0;
             ...
             m.prototype.renderFunnel = function(a) {
                 ...
                 function b() {
                     if (t && x) {
                         ...
                         for (var y = 0, C = x.length - 1; y < x.length || 0 <= C; y++, C--) {
                             ...
                             c.measureText();
                             H.push({ textBlock: c, id: t.reversed ? C : y, isDirty: !1, lineColor: a, lineThickness: n, lineDashType: m, height: c.height < c.maxHeight ? c.height : c.maxHeight, width: c.width < c.maxWidth ? c.width : c.maxWidth })
                         }
                     }
                 }
                 ...
                 function l() {
                     ...
                     function b() {
                         ...
                         f = (T - $ + 2 * D) / m;
                         h = m;
                         for (var l, k = 1; k < h; k++) { e = k * f; for (var p = H.length - 1; 0 <= p; p--) !H[p].isDirty && (H[p].textBlock.y < e && H[p].textBlock.y + H[p].height > e) && (l = q(p), !(l >= H.length - 1) && H[p].textBlock.y + H[p].height + ea > H[l].textBlock.y && (H[p].textBlock.y = H[p].textBlock.y + H[p].height - e > e - H[p].textBlock.y ? e + 1 : e - H[p].height - 1)) }
                 ...
                 function m(a, b, c) {
                     ...
                     var u = { startTime: (new Date).getTime(), duration: c || 500, easingFunction: function(a, b, c, d) { return L.easing.easeOutQuart(a, b, c, d) }, changeSection: function(a) { for (var b, c, d = 0; d < R.length; d++) b = g[d], c = R[d], b *= a, "undefined" === typeof n[d] && (n[d] = 0), 0 > n && (n *= -1), c.y1 += b - n[d], c.y2 += b - n[d], c.y3 += b - n[d], c.y4 += b - n[d], c.y5 && (c.y5 += b - n[d], c.y6 += b - n[d]), n[d] = b } };
                 ...
                 function B(a) { for (var b = 0; b < R.length; b++) H[b].isDirty || (a && (H[b].textBlock.ctx = a), H[b].textBlock.render(!0)) }
                 ...
                 for (var p = 0; p < this.dataSeries.length; p++) {
                     ...
                     if (n.dataPoints && n.dataPoints.length)
                         if ("pie" !== n.type &&
                             ...
                             q = { markerType: q, markerColor: B, text: g, textBlock: null, chartType: n.type, markerSize: s, lineColor: n._colorSet[0], dataSeriesIndex: n.index, dataPointIndex: null, markerBorderColor: v, markerBorderThickness: t };
                             m.push(q)
                         } else
                             for (var x = 0; x < n.dataPoints.length; x++) {
                                 var A = n.dataPoints[x],
                                     ...
                                     g = A.legendText ?
                                     A.legendText : n.legendText ? n.legendText : this.itemTextFormatter ? this.itemTextFormatter({ chart: this.chart, legend: this.options, dataSeries: n, dataPoint: A }) : A.name ? A.name : "DataPoint: " + (x + 1),
                 ...
                 if (0 < m.length) {
                     ...
                     for (p = 0; p < m.length; p++) {
                         ...
                         if (!(0 >= l || "undefined" === typeof l || 0 >= v || "undefined" === typeof v)) {
                             if ("horizontal" ===
                                 this.orientation) {
                                 q.textBlock = new la(this.ctx, { x: 0, y: 0, maxWidth: v, maxHeight: this.itemWrap ? l : this.lineHeight, angle: 0, text: q.text, horizontalAlign: "left", fontSize: this.fontSize, fontFamily: this.fontFamily, fontWeight: this.fontWeight, fontColor: this.fontColor, fontStyle: this.fontStyle, textBaseline: "middle" });
                                 ...
                                 if (!n || n.width +
                                     Math.round(q.textBlock.width + s + k + (0 === n.width ? 0 : this.horizontalSpacing) + ("line" === q.chartType || "spline" === q.chartType || "stepLine" === q.chartType ? 2 * 0.1 * this.lineHeight : 0)) > f) n = { items: [], width: 0 }, h.push(n), this.height += g, g = 0;
                                 ...
                                 fontStyle: this.fontStyle,
                                 textBaseline: "middle"
                             }), q.textBlock.measureText(), null !== this.itemWidth && (q.textBlock.width = this.itemWidth - (s + k + ("line" === q.chartType || "spline" === q.chartType || "stepLine" === q.chartType ? 2 * 0.1 * this.lineHeight : 0))), this.height < l - this.lineHeight ? (n = { items: [], width: 0 }, h.push(n)) : (n = h[x], x = (x + 1) % h.length), this.height += q.textBlock.height;
                         ...
                         if (q) {
                             switch (this.type) {
                                 ...
                                 case "doughnut":
                                     ...
                                     r < q.radius && r > v && (r = Math.atan2(d - s.y, a - s.x), 0 > r && (r += 2 * Math.PI), r = Number(((180 * (r / Math.PI) % 360 + 360) % 360).toFixed(12)), s = Number(((180 * (q.startAngle / Math.PI) % 360 + 360) % 360).toFixed(12)), v = Number(((180 * (q.endAngle / Math.PI) % 360 + 360) % 360).toFixed(12)), 0 === v && 1 < q.endAngle && (v = 360), s >= v && 0 !== g.y && (v += 360, r < s && (r += 360)), r > s && r < v && (b.push({ dataPoint: g, dataPointIndex: f, dataSeries: this, distance: 0 }), k = !0));
                                     break;
                                 case "funnel":
                                 case "pyramid":
                                     r = q.funnelSection;
                                 ...
                                 case "boxAndWhisker":
                                     if (a >= q.x1 - q.borderThickness / 2 && a <= q.x2 + q.borderThickness / 2 && d >= q.y4 - q.borderThickness / 2 && d <= q.y1 + q.borderThickness /
                                         2 || Math.abs(q.x2 - a + q.x1 - a) < q.borderThickness && d >= q.y1 && d <= q.y4) b.push({ dataPoint: g, dataPointIndex: f, dataSeries: this, distance: Math.min(Math.abs(q.x1 - a), Math.abs(q.x2 - a), Math.abs(q.y2 - d), Math.abs(q.y3 - d)) }), k = !0;
                                     break;
                                 case "candlestick":
             ...
             D.prototype.createLabels = function() {
                 ...
                 if ("axisX" !== this.type || "dateTime" !== this.valueType || this.logarithmic) {
                     ...
                     for (this.intervalStartPosition = this.getLabelStartPoint(new Date(this.viewportMinimum), this.intervalType,
                         ...
                         a || (a = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.options, value: new Date(p), label: this.labels[p] ? this.labels[p] : null }) : "axisX" === this.type && this.labels[p] ? this.labels[p] : Aa(p, this.valueFormatString, this.chart._cultureInfo),
                 ...
                 if (this.snapToDataPoint) {
                     ...
                     if ("xySwapped" === this.chart.plotInfo.axisPlacement) {
                         ...
                         if ("bottom" === this.parent._position || "top" === this.parent._position) {
                             ...
                             if (E) {
                                 ...
                                 this.label : "number" === this.parent.valueType && (h.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: c ? c : x.dataPoint.x }) : r(this.options.label) ? fa(c ? c : x.dataPoint.x, this.valueFormatString, this.chart._cultureInfo) : this.label);
                         ...
                         if (E) {
                             if ("axisX" === this.parent.type)
                                 for (s = this.parent.convertPixelToValue({ x: a }),
                         ...
                         if ("rangeColumn" === this.parent.dataSeries[w].type || "rangeArea" === this.parent.dataSeries[w].type || "error" === this.parent.dataSeries[w].type || "rangeSplineArea" === this.parent.dataSeries[w].type || "candlestick" === this.parent.dataSeries[w].type || "ohlc" === this.parent.dataSeries[w].type || "boxAndWhisker" === this.parent.dataSeries[w].type) {
                             ...
                             0, h.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: x.dataPoint.y }) : r(this.options.label) ? fa(c ? c : x.dataPoint.y, this.valueFormatString, this.chart._cultureInfo) : this.label, this.value = x.dataPoint.y);
                         f = l = m;
                         b = this.chart.plotArea.x1;
                         e = this.chart.plotArea.x2;
                         this.bounds = { x1: b, y1: f - n / 2, x2: e, y2: l + n / 2 };
                     ...
                     if ("xySwapped" === this.chart.plotInfo.axisPlacement)
                         if ("left" === this.parent._position || "right" === this.parent._position) {
                             ...
                             if (E) {
                                 if ("axisX" === this.parent.type)
                                     for (s = this.parent.convertPixelToValue({ y: d }), v = null, w = 0; w < this.parent.dataSeries.length; w++)(v = this.parent.dataSeries[w].getDataPointAtX(s, !0)) && 0 <= v.index &&
             ...
             aa.prototype.mouseMoveHandler = function(a, d) { this._lastUpdated && 4 > (new Date).getTime() - this._lastUpdated || (this._lastUpdated = (new Date).getTime(), this.chart.resetOverlayedCanvas(), this._updateToolTip(a, d)) };
                         ...
                         if ("xySwapped" === this.chart.plotInfo.axisPlacement) {
                             ...
                             if (this.chart.axisX)
                                 for (m = 0; m < this.chart.axisX.length; m++)
                                     for (l = this.chart.axisX[m].convertPixelToValue({ x: a }), h = null, b = 0; b < this.chart.axisX[m].dataSeries.length; b++)(h = this.chart.axisX[m].dataSeries[b].getDataPointAtX(l, c)) && 0 <= h.index && (h.dataSeries = this.chart.axisX[m].dataSeries[b], null !== h.dataPoint.y && k.push(h));
                             if (this.chart.axisX2)
                                 for (m = 0; m < this.chart.axisX2.length; m++)
             ...
             aa.prototype.getToolTipInnerHTML = function(a) {
                 ...
                 for (var d = null, c = null, b = null, e = 0, f = "", l = !0, k = 0; k < a.length; k++)
                     if (a[k].dataSeries.toolTipContent ||
                         a[k].dataPoint.toolTipContent) { l = !1; break }
